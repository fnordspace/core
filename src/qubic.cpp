...
// Signing-difficulty target for a voted tick. Pass the tick the vote is FOR
// (etalonTick.tick when signing, request->tick.tick when verifying), not system.tick.
static unsigned int getTargetTickVoteSignature(unsigned int votedTick)
{
    return (votedTick >= SIGNING_DIFFICULTY_SWITCH_TICK)
        ? TARGET_TICK_VOTE_SIGNATURE_NEW
        : TARGET_TICK_VOTE_SIGNATURE_LEGACY;
}

static bool verifyTickVoteSignature(unsigned int votedTick, const unsigned char* publicKey, const unsigned char* messageDigest, const unsigned char* signature, const bool curveVerify = true)
{
    unsigned int score = _byteswap_ulong(((unsigned int*)signature)[0]);
    if (score > getTargetTickVoteSignature(votedTick)) return false;
    if (curveVerify)
    {
...
static void processBroadcastTick(Peer* peer, RequestResponseHeader* header)
{
        ...
        && request->tick.millisecond <= 999)
    {
        ...
        if (verifyTickVoteSignature(request->tick.tick, broadcastedComputors.computors.publicKeys[request->tick.computorIndex].m256i_u8, digest, request->tick.signature, verifyFourQCurve))
        {
            if (header->isDejavuZero())
            {
                enqueueResponse(NULL, header);
...
static void processRequestSystemInfo(Peer* peer, RequestResponseHeader* header)
{
    ...
    respondedSystemInfo.targetTickVoteSignature = getTargetTickVoteSignature(system.tick);
...
static void signTickVote(unsigned int votedTick, const unsigned char* subseed, const unsigned char* publicKey, const unsigned char* messageDigest, unsigned char* signature)
{
    PROFILE_SCOPE();

    signWithRandomK(subseed, publicKey, messageDigest, signature);
    bool isOk = verifyTickVoteSignature(votedTick, publicKey, messageDigest, signature, false);
    while (!isOk)
    {
        signWithRandomK(subseed, publicKey, messageDigest, signature);
        isOk = verifyTickVoteSignature(votedTick, publicKey, messageDigest, signature, false);
    }
}
...
static void broadcastTickVotes()
{
    ...
    for (unsigned int i = 0; i < numberOfOwnComputorIndices; i++)
    {
        ...
        signTickVote(etalonTick.tick, computorSubseeds[ownComputorIndicesMapping[i]].m256i_u8, computorPublicKeys[ownComputorIndicesMapping[i]].m256i_u8, digest, broadcastTick.tick.signature);
