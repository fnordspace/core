...
static void computeTxBodyDigestBase(const int tick)
{
        ...
        if (!isZero(nextTickData.transactionDigests[i]))
        {
            ...
            if (tsTransactionOffsets[i]) {
                ...
                if (transaction->checkValidity() && transaction->tick == tick) {
                    ...
                    if (digest == nextTickData.transactionDigests[i])
                    {
                        copyMem(cur_ptr, reinterpret_cast<const unsigned char*>(transaction), transaction->totalSize());
                        cur_ptr += transaction->totalSize();
                    }
                }
#if !defined(NDEBUG)
                else
    ...
    KangarooTwelve(g_txBodies, static_cast<unsigned int>(cur_ptr - ((unsigned char*)&g_txBodies)), reinterpret_cast<unsigned char*>(&etalonTick.saltedTransactionBodyDigest), 4);
}
